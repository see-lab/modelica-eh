...
model HeatPump_y
  "Heat pump with ideal control that takes load input signal for the load (condenser side) heat flow rate"
  extends EnergyHub.Subsystems.BaseClasses.PartialThermalSubsystem(
    final heaMod=true,
    final mDis_flow_nominal=heaPum.m2_flow_nominal,
    final mLoa_flow_nominal=heaPum.m1_flow_nominal,
    sumPow(nin=3),
    pumLoa(show_T=true),
    loa(show_T=true));
...
//    "Evaporator mass flow rate";
  Buildings.Fluid.HeatPumps.Carnot_TCon heaPum(
    ...
    show_T=true,
    QCon_flow_nominal=QLoa_flow_nominal,
  ...
  Buildings.Fluid.Movers.Preconfigured.SpeedControlled_y pumDis(
    ...
    "Pump for heat pump district (evaporator) side" annotation (Placement(
        transformation(
        extent={{-10,10},{10,-10}},
        rotation=180,
        origin={10,-10})));
  ...
  Buildings.Fluid.Actuators.Valves.ThreeWayEqualPercentageLinear valHeaPumEva(
    redeclare package Medium = MediumDis,
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState,
    m_flow_nominal=mDis_flow_nominal,
  ...
  Modelica.Blocks.Sources.Constant TSet(k=TCon_nominal)
    "Condenser water temperature setpoint"
    annotation (Placement(transformation(extent={{-100,-80},{-80,-60}})));
equation
  connect(loa.port_b, heaPum.port_a1) annotation (Line(points={{-30,80},{-46,80},
          {-46,10},{-40,10}}, color={0,127,255}));
  ...
  connect(conPI.y,valHeaPumEva. y) annotation (Line(points={{42,-80},{52,-80},{
          52,6},{40,6},{40,2}},    color={0,0,127}));
  connect(senTDisRet.T,dT_supRet. u2) annotation (Line(points={{-60,-21},{-60,
          -126},{-22,-126}},
                          color={0,0,127}));
  connect(senTDisSup.port_a,senMasFlo. port_b)
    annotation (Line(points={{80,-10},{86,-10},{86,-10},{90,-10}},
                                                   color={0,127,255}));
  connect(valHeaPumEva.port_1,senTDisSup.port_b)
    annotation (Line(points={{50,-10},{56,-10},{56,-10},{60,-10}},
                                                  color={0,127,255}));
  connect(jun.port_3, valHeaPumEva.port_3) annotation (Line(points={{-90,-20},{
          -90,-40},{40,-40},{40,-20}}, color={0,127,255}));
  connect(valHeaPumEva.port_2,pumDis. port_a)
  ...
  connect(pumDis.port_b, heaPum.port_a2) annotation (Line(points={{0,-10},{-10,-10},
          {-10,-2},{-20,-2}}, color={0,127,255}));
  connect(heaPum.port_b2,senTDisRet.port_a)  annotation (Line(points={{-40,-2},
          {-46,-2},{-46,-10},{-50,-10}},
                                    color={0,127,255}));
  ...
  connect(TSet.y, heaPum.TSet) annotation (Line(points={{-79,-70},{-48,-70},{
          -48,13},{-42,13}}, color={0,0,127}));
  connect(heaPum.port_b1, pumLoa.port_a) annotation (Line(points={{-20,10},{36,10},
          {36,80},{30,80}}, color={0,127,255}));
  ...
  connect(heaPum.P, sumPow.u[3])
    annotation (Line(points={{-19,4},{0,4},{0,50},{78,50}}, color={0,0,127}));
